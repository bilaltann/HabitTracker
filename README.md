...
```bash
git clone https://github.com/bilaltann/HabitTracker
cd HabitTracker
```

**2.** `HabitTracker.API/appsettings.json` dosyasını kendi veritabanı, Google OAuth ve mail ayarlarınıza göre yapılandırın.

**3.** Docker compose ile tüm sistemi ayağa kaldırın:

```bash
docker-compose up -d --build
```

**4.** Veritabanı tablolarını oluşturmak için Entity Framework migration'larını uygulayın (Package Manager Console):
...
Sistem başarıyla ayağa kalktığında uygulamaya `http://[AWS-PUBLIC-IP]` adresi üzerinden erişilebilir.
